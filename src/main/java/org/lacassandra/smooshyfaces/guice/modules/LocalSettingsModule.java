...
public class LocalSettingsModule extends AbstractModule {

    @Override
    protected void configure() {
        // Astyanax/Cassandra Settings
        bind(String.class).annotatedWith(Names.named("SEED_HOSTS")).toInstance("localhost:9160");
        bind(String.class).annotatedWith(Names.named("CASSANDRA_CLUSTER_NAME")).toInstance("MyCluster");
        ...
        bind(String.class).annotatedWith(Names.named("API_SERVERS")).toInstance("localhost");

        // Paging
        bindConstant().annotatedWith(Names.named("DEFAULT_PAGE_SIZE")).to(1);
